// Package shutdown provides a way to shutdown server gracefully.
package shutdown
...
func (h *Helper) Wait(srv io.Closer) error {
	<-h.signals
	str := time.Now()
	log.Info("received interrupt signal, shutting down...", log.Any("time", str))
	...
	log.Info("server shut down ", log.Any("time", str))
	return nil
}
