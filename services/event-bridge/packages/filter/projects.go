...
const specials = `[
  ...
  {
    "url": "https://vrt-wiki.wikimedia.org",
...
// NewProjects creates new projects filter.
func NewProjects() (*Projects, error) {
	list := []struct {
		DbName string `json:"dbname"`
	}{}

	if err := json.NewDecoder(strings.NewReader(specials)).Decode(&list); err != nil {
		return nil, err
	}

	projects := new(Projects)
	projects.specials = make(map[string]struct{})

	for _, project := range list {
		projects.specials[project.DbName] = struct{}{}
	}

	return projects, nil
}

// Projects filter to check whether we support this particular project.
type Projects struct {
	specials map[string]struct{}
}

// IsSupported check if the project database name is supported by our system.
func (p *Projects) IsSupported(dbName string) bool {
	_, ok := p.specials[dbName]
	return !ok
}
