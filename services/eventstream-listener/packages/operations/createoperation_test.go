...
type createNewTestSuite struct {
	suite.Suite
	env *env.Environment
	fr  *filter.Filter
...
func (s *createNewTestSuite) SetupSuite() {
...
func (s *createNewTestSuite) TestNew() {
	s.Assert().NotNil(operations.NewCreateOperation(s.trs, s.fr, s.env))
}

func TestCreateNew(t *testing.T) {
	suite.Run(t, new(createNewTestSuite))
}

type createExecuteTestSuite struct {
	suite.Suite
	env      *env.Environment
	fr       *filter.Filter
	trs      *transformer.Transforms
	create   operations.Operation
	msgCount int
	topic    string
...
func (s *createExecuteTestSuite) SetupTest() {
	s.env = new(env.Environment)
	...
	s.create = operations.NewCreateOperation(s.trs, s.fr, s.env)
}

func (s *createExecuteTestSuite) TestExecute() {
	msgs, err := s.create.Execute(s.ctx, s.event)
...
func TestCreateExecute(t *testing.T) {
	...
	for _, testcase := range []*createExecuteTestSuite{
		{
			msgCount: 0,
			event:    testEventA,
		},
		...
		},
		{
			msgCount: 1,
			event:    testEventE,
			topic:    "aws.event-bridge.commons-create.v1",
		},
	} {
