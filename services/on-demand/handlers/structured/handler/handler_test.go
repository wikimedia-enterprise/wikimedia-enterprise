...
func (m *unmarshalerMock) Unmarshal(_ context.Context, data []byte, v interface{}) error {
	...
	if strings.Contains(typ.String(), "AvroStructured") {
		*v.(*schema.AvroStructured) = *ags.Get(0).(*schema.AvroStructured)
	} else {
		*v.(*schema.Structured) = *ags.Get(0).(*schema.Structured)
	}
...
func (s *handlerTestSuite) SetupSuite() {
	...
	val, _ := art.ToJsonStruct()
	s.stg.On("Update", s.msg.Key, etp, val).Return(s.ste)
